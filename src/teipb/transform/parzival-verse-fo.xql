...
module namespace model="http://www.tei-c.org/pm/models/parzival-verse/fo";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(milestone) return
                        if (@unit='Versumstellung') then
                            ...
                            if (@unit='Nicht_ausgeführtes_Bild') then
                                ...
                                return
                                                                fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone2", "image", css:map-rend-to-class(.)), $content)
                            else
                                if (@unit='Nicht_ausgeführtes_Bild_mit_Nachtrag') then
                                    ...
                                    return
                                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone3", "image", css:map-rend-to-class(.)), $content)
                                else
                                    if (@unit='Bild') then
                                        let $params := 
                                            map {
                                        ...
                                        return
                                                                                fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone4", "image", css:map-rend-to-class(.)), $content)
                                    else
                                        fo:inline($config, ., ("tei-milestone5", "milestone", css:map-rend-to-class(.)), .)
                    case element(l) return
                        ...
                        return
                                                fo:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", "line", css:map-rend-to-class(.))), $content)
