...
module namespace model="http://www.tei-c.org/pm/models/parzival-verse/web";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(milestone) return
                        if (@unit='Versumstellung') then
                            ...
                            if (@unit='Nicht_ausgeführtes_Bild') then
                                ...
                                return
                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone2", "image", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                if (@unit='Nicht_ausgeführtes_Bild_mit_Nachtrag') then
                                    ...
                                    return
                                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone3", "image", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds)
                                else
                                    if (@unit='Bild') then
                                        let $params := 
                                            map {
                                        ...
                                        return
                                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone4", "image", css:map-rend-to-class(.)), $content)                                        => model:map($node, $trackIds)
                                    else
                                        html:inline($config, ., ("tei-milestone5", "milestone", css:map-rend-to-class(.)), .)                                        => model:map($node, $trackIds)
                    case element(l) return
                        ...
                        return
                                                html:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", "line", css:map-rend-to-class(.))), $content)                        => model:map($node, $trackIds)
                    ...
                    case element(cb) return
                        ...
                        return
                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-cb", css:map-rend-to-class(.)), $content)                        => model:map($node, $trackIds)
