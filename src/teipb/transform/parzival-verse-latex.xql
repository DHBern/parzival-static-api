(:~
    ...
    ODD: /db/apps/parzival/odd/parzival-verse.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/parzival-verse/latex";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(l) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", "line", css:map-rend-to-class(.))), $content)
                    ...
                    case element(lb) return
                        ...
                        return
                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-lb", css:map-rend-to-class(.))), $content)
                    ...
                    case element(note) return
                        if (@resp) then
                            ...
                            if (@type='Notiz') then
                                latex:inline($config, ., ("tei-note2", "note", css:map-rend-to-class(.)), .)
                            else
                                if (@type='Marginalie') then
                                    latex:inline($config, ., ("tei-note3", "marginalia", css:map-rend-to-class(.)), .)
                                else
                                    if (not(@type='Kapitelüberschrift')) then
                                        latex:inline($config, ., ("tei-note4", "note", css:map-rend-to-class(.)), .)
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(del) return
                        if (@hand='#sr') then
                            latex:inline($config, ., ("tei-del1", "del_sr", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-del2", "deleted", css:map-rend-to-class(.)), .)
                    ...
                    case element(add) return
                        if (@hand="#sr") then
                            latex:inline($config, ., ("tei-add1", "sr", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-add2", "added", css:map-rend-to-class(.)), .)
                    ...
                    case element(subst) return
                        if (@hand[starts-with(., '#ls')]) then
                            ...
                            if (@hand[starts-with(., '#sr')]) then
                                latex:inline($config, ., ("tei-subst2", "subst_sr", css:map-rend-to-class(.)), .)
                            else
                                latex:inline($config, ., ("tei-subst3", "subst", css:map-rend-to-class(.)), .)
                    ...
                    case element(corr) return
                        if (parent::choice and count(parent::*/*) gt 1) then
                            ...
                            latex:inline($config, ., ("tei-corr2", "corr", css:map-rend-to-class(.)), .)
                        ...
                        if (number(@extent)=1) then
                            ...
                            if (number(@extent)=2) then
                                ...
                                if (@reason="Fragmentverlust") then
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-gap3", "fragment-loss", css:map-rend-to-class(.)), $content)
                                else
                                    if (@extent="unbekannt" or number(@extent)>=3) then
                                        ...
                                        return
                                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-gap5", "gap", css:map-rend-to-class(.)), $content)
                                ...
                                if (@type='kleine_Variante') then
                                    ...
                                    if (@type='singuläre_Lesart') then
                                        ...
                                        if (@type='große_Variante') then
                                            latex:inline($config, ., ("tei-seg5", "large-variant", css:map-rend-to-class(.)), .)
                                        else
                                            if (@subtype='Großinitiale') then
                                                latex:inline($config, ., ("tei-seg6", "capital-initial", css:map-rend-to-class(.)), .)
                                            else
                                                if (@subtype='Majuskel') then
                                                    latex:inline($config, ., ("tei-seg7", "majuscule", css:map-rend-to-class(.)), .)
                                                else
                                                    if (@subtype='Prachtinitiale') then
                                                        latex:inline($config, ., ("tei-seg8", "glory-initial", css:map-rend-to-class(.)), .)
                                                    else
                                                        if (@type='Initiale') then
                                                            latex:inline($config, ., css:get-rendition(., ("tei-seg9", "initial", css:map-rend-to-class(.))), .)
                                                        else
                                                            $config?apply($config, ./node())
                    ...
                    case element(cb) return
                        ...
                        return
                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-cb", css:map-rend-to-class(.)), $content)
