...
module namespace model="http://www.tei-c.org/pm/models/parzival-verse/print";
    ...
    $input !         (
            ...
            return
                            ...
                            if (@unit='Nicht_ausgeführtes_Bild') then
                                ...
                                if (@unit='Nicht_ausgeführtes_Bild_mit_Nachtrag') then
                                    ...
                                    if (@unit='Bild') then
                                        ...
                                        return
                                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-milestone4", "image", css:map-rend-to-class(.)), $content)
                                    else
                                        html:inline($config, ., ("tei-milestone5", "milestone", css:map-rend-to-class(.)), .)
                    case element(l) return
                        ...
                        return
                                                html:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", "line", css:map-rend-to-class(.))), $content)
                    ...
                    case element(choice) return
                        if (am and ex) then
                            ...
                            if (sic and corr) then
                                printcss:alternate($config, ., ("tei-choice2", css:map-rend-to-class(.)), ., corr[1], sic[1])
                            else
                                if (abbr and expan) then
                                    printcss:alternate($config, ., ("tei-choice3", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                                else
                                    if (orig and reg) then
                                        printcss:alternate($config, ., ("tei-choice4", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(cb) return
                        ...
                        return
                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-cb", css:map-rend-to-class(.)), $content)
